...
if __name__ == "__main__":
    ...
    for task in test_tasks:
        ...
        if args.llm_model == "gpt":
            if "gpt" not in args.gpt_version:
                # older gpt versions
                _, text = LM(curr_prompt, args.llm_model, args.gpt_version, max_tokens=1000, stop=["def"], frequency_penalty=0.15)
            else:            
                messages = [{"role": "user", "content": curr_prompt}]
                _, text = LM(messages, args.llm_model, args.gpt_version, max_tokens=1300, frequency_penalty=0.0)
        elif args.llm_model == "gemini":
            _, text = LM(curr_prompt, args.llm_model,args.gemini_model, max_tokens=1000, stop=["def"], frequency_penalty=0.15)
    ...
    for i, plan in enumerate(decomposed_plan):
        ...
        if args.llm_model == "gpt":
            if "gpt" not in args.gpt_version:
                ...
                messages = [{"role": "user", "content": curr_prompt}]
                _, text = LM(messages, args.llm_model, args.gpt_version, max_tokens=1500, frequency_penalty=0.35)
                ...
                _, text = LM(messages,args.llm_model, args.gpt_version, max_tokens=400, frequency_penalty=0.69)
        elif args.llm_model == "gemini":
            ...
            _, text = LM(messages, args.llm_model, args.gemini_model, max_tokens=1000, stop=["def"], frequency_penalty=0.69)
    ...
    for i, (plan, solution) in enumerate(zip(decomposed_plan,allocated_plan)):
        ...
        if args.llm_model == "gpt":
            if "gpt" not in args.gpt_version:
                # older versions of GPT
                _, text = LM(curr_prompt, args.llm_model, args.gpt_version, max_tokens=1000, stop=["def"], frequency_penalty=0.30)
            else:            
                ...
                _, text = LM(messages, args.llm_model, args.gpt_version, max_tokens=1400, frequency_penalty=0.4)
        elif args.llm_model == "gemini":
            _, text = LM(curr_prompt, args.llm_model, args.gemini_model, max_tokens=1000, stop=["def"], frequency_penalty=0.15)
