...
client = OpenAI(api_key=Path('deepseek_api_key.txt').read_text(), base_url="https://api.deepseek.com")
...
def LM(prompt, ds_version, max_tokens=128, temperature=0, stop=None, logprobs=1, frequency_penalty=0):
    
    if "deepseek" not in ds_version:
        response = client.Completion.create(model=ds_version, 
                                            prompt=prompt, 
        ...
        response = client.chat.completions.create(model=ds_version, 
                                            messages=prompt, 
                                            max_tokens=max_tokens, 
                                            temperature=temperature, 
                                            frequency_penalty = frequency_penalty,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--ds-version", type=str, default="deepseek-chat", 
    ...
    for task in test_tasks:
        ...
        # if "gpt" not in args.ds_version:
        #     # older gpt versions
        #     _, text = LM(curr_prompt, args.ds_version, max_tokens=1000, stop=["def"], frequency_penalty=0.15)
        # else:            
        messages = [{"role": "user", "content": curr_prompt}]
        _, text = LM(messages, args.ds_version, max_tokens=1300, frequency_penalty=0.0)
    ...
    for i, plan in enumerate(decomposed_plan):
        ...
        #     _, text = LM(curr_prompt, args.ds_version, max_tokens=1000, stop=["def"], frequency_penalty=0.65)
        
        # elif "gpt-3.5" in args.ds_version:
        #     # gpt 3.5 and its variants
        #     messages = [{"role": "user", "content": curr_prompt}]
        #     _, text = LM(messages, args.ds_version, max_tokens=1500, frequency_penalty=0.35)
        ...
        _, text = LM(messages, args.ds_version, max_tokens=400, frequency_penalty=0.69)
    ...
    for i, (plan, solution) in enumerate(zip(decomposed_plan,allocated_plan)):
        ...
        # if "gpt" not in args.ds_version:
        #     # older versions of GPT
        #     _, text = LM(curr_prompt, args.ds_version, max_tokens=1000, stop=["def"], frequency_penalty=0.30)
        # else:            
        ...
        _, text = LM(messages, args.ds_version, max_tokens=1400, frequency_penalty=0.4)
